...
var AudioModule = ContainerModule.extend({
  // @Override
  defaults: function() {
    return _.defaults({
  ...
  onAfterChildAdd: function(child) {
    ContainerModule.prototype.onAfterChildAdd.apply(this, arguments);
  ...
  onAfterChildRemove: function(child) {
    ContainerModule.prototype.onAfterChildRemove.apply(this, arguments);
    this._updatePositionAndSize();
  },

  _updatePositionAndSize: function() {
    var length = this.get('children').length;
    this.resize(new Size({
      width: 100 * length,
